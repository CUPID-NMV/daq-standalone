...
class Monitor:
    ...
    def figure(self, kind, n_show=25):
        res = self.analysis()
        if res is None:
        ...
        if kind == "waveforms":
            fig, axes = plt.subplots(len(channels), 1, figsize=(9, 2.6 * len(channels)),
                                     squeeze=False, sharex=True)
            n = min(n_show, corr.shape[0])
            for i, ch in enumerate(channels):
                ax = axes[i][0]
                for e in range(corr.shape[0] - n, corr.shape[0]):
                    ax.plot(t_ns, corr[e, i], lw=0.6, alpha=0.5)
                ax.axhline(0, color="k", lw=0.8, ls=":")
                ax.set_title(f"ch{ch} — ultimi {n} eventi", fontsize=10)
...
async function tick() {
  try {
    ...
    for (const [id, name] of [['w','waveforms'],['a','average'],['h','amplitudes']])
      document.getElementById(id).src = name + '.png?t=' + t;
...
def make_handler(monitor, refresh, n_show):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            route = self.path.split("?")[0].lstrip("/") or "index.html"
            ...
            with monitor.lock:
                ...
                if route in kinds:
                    return self._send(200, "image/png",
                                      monitor.figure(kinds[route], n_show))
...
def main():
    ...
    ap.add_argument("-n", "--nevents", type=int, default=25,
                    help="eventi sovrapposti nel grafico (default 25)")
    ...
    server = ThreadingHTTPServer(("127.0.0.1", args.port),
                                 make_handler(monitor, args.refresh, args.nevents))
