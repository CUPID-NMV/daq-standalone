...
def fig_efficienza(meta, amp, pos, out):
    ...
    for x in lim:
        ax.annotate("", xy=(x["distanza"], 0.12), xytext=(x["distanza"], 0.9),
    ...
    k = a50 / d50                       # conteggi di ampiezza per conteggio di distanza
    griglia = np.linspace(0, d.max() + 1, 400)
    prev = np.array([(reali < -k * g).mean() for g in griglia])
    ...
    ax.annotate("50%%", xy=(d.max() + 0.6, 52), color="#666", fontsize=9)
...
def fig_deriva(meta, amp, pos, dc, chans, out):
    ...
    for i, ch in enumerate(chans):
        ax.plot(ev, dc[:, i] - dc[:200, i].mean(), lw=.9,
                label="ch%d%s" % (ch, "  (collegato al generatore)" if i == 0
                                  else "  (non collegato)"))
    ax.plot(ev, (dc[:, 0] - dc[:200, 0].mean()) - (dc[:, 1] - dc[:200, 1].mean()),
            lw=1.6, color="#c0392b", label="differenza  (parte non comune)")
